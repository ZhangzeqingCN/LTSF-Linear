...
class MyDataset(Dataset):
    ...
    def __init__(self, P, F):
        ...
        self.values = (values - np.min(values)) / (np.max(values) - np.min(values))
...
class KANLinear(nn.Module):
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        ...
        behind the f.linear function if we want n memory efficient implementation.
...
@paprika.timeit
def train(model: nn.Module, cfg: Config):
    ...
    for _ in (range(cfg.num_epochs)):
        ...
        for X_batch, Y_batch in tqdm(DataLoader(
                MyDataset(P=cfg.P, F=cfg.F),
                batch_size=cfg.B,
                # generator=torch.Generator(device=cfg.device),
                num_workers=6,
        )):
            X_batch = X_batch.permute(0, 2, 1).to(cfg.device)
            Y_batch = Y_batch.permute(0, 2, 1).to(cfg.device)
            optimizer.zero_grad()
...
def train_mlp_and_kan():
    cfg = Config(use_cuda=True)
    logger.info(F"{cfg.device=}")
    torch.set_default_device(cfg.device)
    torch.set_default_dtype(torch.double)
    train(nn.Linear(cfg.P, cfg.F), cfg)
    # train(KAN([cfg.P, 2 * max(cfg.P, cfg.F), cfg.F]), cfg)
